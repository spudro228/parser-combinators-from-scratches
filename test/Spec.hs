...
main =
  hspec $ do
    ...
    describe "\"anyOf\" parse any character from list in string" $ do
      ...
      it "anyOf number 9 from ['0' .. '9'] in 9abc" $ do (run (anyOf ['0' .. '9']) "9abc") `shouldBe` Success ("9", "abc")
      it "anyOf number 9 from ['0' .. '9'] in 96abc" $ do (run (anyOf ['0' .. '9']) "96abc") `shouldBe` Success ("9", "6abc")
      it "dont choice number 9 from ['0' .. '9']" $ do (run (anyOf ['0' .. '9']) "|bc") `shouldBe` (Failure "Expect 'a' but got '|'")
